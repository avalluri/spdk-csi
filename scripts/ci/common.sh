...
function prepare_xpu_node() {
	...
	sleep 10s
	sudo docker exec -i "${SPDK_XPU_CONTAINER}" /root/spdk/scripts/rpc.py nvmf_create_transport -t VFIOUSER
	sudo docker exec -i "${SPDK_XPU_CONTAINER}" /root/spdk/scripts/rpc.py nvmf_create_transport -t TCP
    sleep 10s
