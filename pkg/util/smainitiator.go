...
func NewSpdkCsiSmaInitiator(volumeContext map[string]string, xpuClient *grpc.ClientConn, trType TransportType) (XpuInitiator, error) {
...
func (i *smaInitiatorNvmfTCP) Connect(ctx context.Context, params *ConnectParams) error {
	volumeID, err := volumeUUID(i.volumeContext["model"])
	if err != nil {
		return err
	}
...
// For SMA NvmfTCP Disconnect(), ,
// then, DetachVolume() will be called to detach the volume from the device,
